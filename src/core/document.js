...
import {mergeBounds} from './math.js';
let serial=0;
...
export class ModelDocument extends EventTarget {
  constructor(){super();this.name='Untitled design';this.bodies=[];this.sketches=[];this.components=[{id:'parts',name:'Parts'}];this.layers=[{id:'default',name:'Default',visible:true}];this.groups=[];this.units='mm';this.revision=0;this.history=[];this.cursor=0;this.savedRevision=-1;this.maxHistory=60;}
  snapshot(){return {name:this.name,bodies:this.bodies.map(b=>({...b})),sketches:structuredClone(this.sketches),components:structuredClone(this.components),layers:structuredClone(this.layers),groups:structuredClone(this.groups),units:this.units};}
  restore(s){Object.assign(this,{...s,bodies:s.bodies.map(b=>({...b})),sketches:structuredClone(s.sketches),components:structuredClone(s.components),layers:structuredClone(s.layers),groups:structuredClone(s.groups)});}
  ...
  remove(ids){const set=new Set(ids);this.bodies=this.bodies.filter(b=>!set.has(b.id));this.groups=this.groups.map(g=>({...g,ids:g.ids.filter(id=>!set.has(id))}));}
  serialize(){return {format:'avolith',version:1,name:this.name,units:this.units,bodies:this.bodies.map(b=>({...b,solid:b.solid.toJSON()})),sketches:this.sketches,components:this.components,layers:this.layers,groups:this.groups};}
  static parse(input){
  ...
  report(){const rows=this.bodies.map(b=>{const m=metrics(b.solid),material=MATERIALS[b.material];return {id:b.id,name:b.name,material:material.name,volume_mm3:m.volume,area_mm2:m.area,mass_g:m.volume*material.density,centroid_mm:m.centroid,triangles:m.triangles};});return {document:this.name,units:'mm',geometry:'faceted',bodies:rows,totalMass_g:rows.reduce((s,b)=>s+b.mass_g,0)};}
}
